...
CURRENT_MODEL = "bybit-taxonomy-v9-selected-policy-terminal-cohorts"
...
def test_model_and_calibrator_lineage_are_advanced() -> None:
    ...
    assert calibration.BOT_CALIB_KEYS["futures_grid"] == "logreg_futures_grid_v20"
    assert calibration.GLOBAL_LOGREG_KEY == "logreg_global_v20"
...
def test_status_separates_historical_current_and_feature_eligible_outcomes(tmp_path: Path, monkeypatch) -> None:
    ...
    try:
        ...
        assert status["global_calibrator_base_key"] == "logreg_global_v20"
        assert status["global_calibrator_key"].startswith("logreg_global_v20:")
        ...
        assert bot["calibrator_base_key"] == "logreg_futures_grid_v20"
        assert bot["calibrator_key"].startswith("logreg_futures_grid_v20:")
