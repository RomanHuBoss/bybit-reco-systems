...
def test_model_and_calibrator_lineage_are_advanced() -> None:
    ...
    assert calibration.GLOBAL_LOGREG_KEY == "logreg_global_v23"
    assert recommender.DIRECTION_CALIBRATION_KEY == "platt_direction_v15"
...
def test_status_separates_historical_current_and_feature_eligible_outcomes(tmp_path: Path, monkeypatch) -> None:
    ...
    try:
        ...
        assert status["global_calibrator_base_key"] == "logreg_global_v23"
        assert status["global_calibrator_key"].startswith("logreg_global_v23:")
        ...
        assert bot["calibrator_base_key"] == "logreg_futures_grid_v23"
        assert bot["calibrator_key"].startswith("logreg_futures_grid_v23:")
