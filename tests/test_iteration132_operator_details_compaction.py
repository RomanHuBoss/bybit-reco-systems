...
def test_details_panel_is_operator_first_not_diagnostic_dump() -> None:
    ...
    assert "Можно готовить запуск" in app_js
    assert "Поля для Bybit" in app_js
    assert "Контроль запуска" in app_js
...
def test_details_panel_keeps_launch_blockers_visible() -> None:
    ...
    assert "Создание бота оператором должно быть остановлено" in app_js
    assert "Ошибки tick/lot/min-notional/LinearPerpetual блокируют ручной запуск" in app_js
...
def test_static_asset_cache_key_bumped_after_details_compaction() -> None:
    ...
    assert "styles.css?v=manual-ui-v17" in index
    assert "app.js?v=manual-ui-v17" in index
