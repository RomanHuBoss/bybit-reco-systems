...
def test_stale_positive_bot_calibrator_is_not_kept_when_current_evidence_is_insufficient(
    ...
    calibration.save_logreg_to_db(
        conn,
        calibration.BOT_CALIB_KEYS["futures_grid"],
        _positive_logreg(),
    )
    monkeypatch.setattr(
        recommender,
    ...
    loaded = recommender._load_or_fit_bot_logregs(conn, min_samples=80)["futures_grid"]
    ...
    persisted = calibration.load_logreg_from_db(
        conn, calibration.BOT_CALIB_KEYS["futures_grid"]
    )
...
def test_stale_positive_global_calibrator_is_not_kept_when_current_evidence_is_insufficient(
    ...
    db.init_db(conn)
    calibration.save_logreg_to_db(conn, calibration.GLOBAL_LOGREG_KEY, _positive_logreg())
    monkeypatch.setattr(recommender, "_fit_global_logreg", lambda *_args, **_kwargs: _insufficient_logreg())

    loaded = recommender._load_or_fit_global_logreg(conn, min_samples=80)

    assert loaded.fitted is False
    assert loaded.expectancy_status == "insufficient"
    assert loaded.n_samples == 12
    persisted = calibration.load_logreg_from_db(conn, calibration.GLOBAL_LOGREG_KEY)
    assert persisted is not None
    assert persisted.fitted is False
    assert persisted.expectancy_status == "insufficient"
    conn.close()
...
def test_stale_direction_calibrator_is_not_kept_when_current_evidence_is_insufficient(
    ...
    loaded = recommender._load_or_fit_direction_calibrator(conn, min_samples=80)

    assert loaded.fitted is False
    persisted = calibration.load_platt_from_db(conn, recommender.DIRECTION_CALIBRATION_KEY)
    assert persisted is not None
    assert persisted.fitted is False
    conn.close()
