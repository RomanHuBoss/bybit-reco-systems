...
def test_results_keeps_only_strategy_badge_table_in_primary_section() -> None:
    source = APP_JS.read_text(encoding="utf-8")
    start = source.index('<div class="modal-section-title">Стратегии</div>')
    end = source.index('<div class="modal-section-title">На что стоит смотреть в первую очередь</div>', start)
    ...
    assert "Тип события · всего" not in section


def test_health_and_outcomes_use_1900px_wide_modal_contract() -> None:
    source = APP_JS.read_text(encoding="utf-8")
    ...
    assert 'width: min(1900px, calc(100vw - 20px));' in styles
    assert 'height: calc(100vh - 20px);' in styles
    assert ".modal-table-two-column" in styles
    assert ".modal-table-many-columns" in styles
    assert "maxHeight: 680" in source
    assert "maxHeight: 640" in source
    assert "ui=wide-observability-v1" in index
