...
def test_auto_snap_publishes_worst_case_grid_notional_and_margin(app_main) -> None:
    ...
    assert sizing["estimated_worst_case_total_order_notional_usdt"] == pytest.approx(1200.0)
    assert sizing["estimated_worst_case_margin_required_usdt"] == pytest.approx(240.0)
    assert economics["estimated_max_position_notional_usdt"] == pytest.approx(1200.0)
    assert operator_economics["capital_required_usdt"] == pytest.approx(240.0)
    assert operator_economics["estimated_worst_case_margin_required_usdt"] == pytest.approx(240.0)
