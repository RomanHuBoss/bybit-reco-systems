...
## Что дополнительно усилено в iteration 63
- Исправлен cold-start разрыв целостности для derived TF: `15m` / `30m` / `4h` теперь могут получить одноразовый REST bootstrap, если локально ещё нет достаточной истории для multi-timeframe logic. Это устраняет ситуацию, когда после свежего старта движок формально «поддерживает» TF, но фактически несколько часов работает без части голосов.
- Сбор `open interest` стал gap-aware: после простоя или длинного цикла collector больше не просит фиксированный хвост `limit=6`, а рассчитывает окно догрузки от последнего локального timestamp с overlap. Это уменьшает риск тихого разрыва OI-истории и искажения `oi_4h` / `oi_24h` сигналов.
- `health/symbols` теперь считает `disabled` по ещё активному `retry_at`, а не по любому старому `SYMBOL_DISABLED` событию за последние 24 часа. Статус и флаг `disabled` снова согласованы между собой и не «залипают» после истечения retry window.
- Добавлены новые регрессионные тесты на retry/backoff клиента Bybit, forward `start/end` для OI, bootstrap derived TF, gap-catchup OI и истечение disabled-state.
- Закрыты предупреждения о незакрытых SQLite connections в тестах кастомных временных БД.
...
- `95 passed`
- покрытие `app/*` — `71%`
- отдельные регрессионные тесты добавлены именно на collector / Bybit client / health semantics
