...
- панель деталей переписана под ручной запуск: JSON убран из основной операторской зоны; вместо него выводятся копируемые поля именно под форму Bybit для конкретного типа бота (`диапазон`, `сетки`, `интервал`, `плечо`, `TP/SL` или верхняя/нижняя стоп-цена); ссылки на график и страницу создания бота вынесены только в заголовок панели деталей и в таблицу;
