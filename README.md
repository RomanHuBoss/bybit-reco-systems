...
Изменение feature semantics начинает новую lineage: `bybit-taxonomy-v13-log-symmetric-direction`, `platt_direction_v15`, `logreg_futures_grid_v23`, `logreg_directional_trend_v4`, `logreg_global_v23`, `directional-trend-v6`, `trend-first-touch-softmax-v3`. Старые recommendations/outcomes остаются immutable audit archive и не должны объединяться с новой exact-policy когортой.

Окно «Результаты наблюдений» больше не называет строки БД независимыми испытаниями. API и UI отдельно показывают число строк, уникальные времена старта, символы, связанные группы перекрывающихся горизонтов и максимальное число неперекрывающихся окон. Основная strategy-таблица использует `by_bot_cohort`, поэтому `calibration_eligible`, `policy_evaluation`, `shadow_exploration` и другие mutually exclusive когорты не смешиваются в одной строке.

Окно «Журнал решений» полностью переработано: вместо узкой девятиколоночной таблицы и сырого JSON каждая запись отображается широкой master-detail карточкой. Время, локализованное действие, символ, strategy family, направление, статус и краткая причина видны сразу; полный payload раскрывается структурированным списком. Диалог сохраняет wide-контракт, отдельную прокрутку, Escape, backdrop и кнопку закрытия.

Обновление не меняет схему SQLite/PostgreSQL и не требует очистки БД. Остановите прежний процесс, сделайте резервную копию БД, замените файлы приложения и перезапустите сервис для загрузки новых frontend assets и model lineage. Проект остаётся recommendation/audit service и не выставляет биржевые ордера.

## Исполнимый sizing и математика exit-path (v1.4.6)

Версия 1.4.6 исправляет причину блокировки, показанную в карточках `ORDER_QTY_BELOW_MIN` / `ORDER_QTY_OFF_STEP`. Генератор рассчитывал provisional quantity из целевого номинала порядка 25 USDT, но для дорогого BTC этот размер мог быть меньше одного биржевого шага `minOrderQty=qtyStep=0.001`. Прежний общий запрет на повышение qty был корректен для ручного размера, но делал автоматически сформированный план заведомо неисполнимым.

Теперь действуют две разные семантики. Явный operator/manual qty никогда не увеличивается и при несоответствии блокируется. Generated provisional qty разрешено поднять только до **минимального** значения, одновременно удовлетворяющего live `minOrderQty`, `qtyStep` и `minNotionalValue`. После этого система заново считает количество активных/коммитированных grid slots, worst-case position notional и margin и повторно применяет runtime limits и daily-loss budget. Если минимально исполнимая сетка не помещается в риск-профиль, она остаётся `НЕ ТОРГОВАТЬ`, но уже с содержательной причиной лимита, а не с каскадом производных qty-ошибок. Generated leverage выравнивается только вниз.

В UI одинаковый текст больше не показывается одновременно под конкретным кодом и generic `RISK`. Для qty ниже биржевого минимума первичной причиной остаётся `ORDER_QTY_BELOW_MIN`; производная `ORDER_QTY_OFF_STEP` не добавляется, когда единственное допустимое округление вниз равно нулю.

Дополнительно исправлена first-touch математика directional outcome. После gap/TP/SL выхода неизвестные более поздние high/low той же минутной свечи не включаются в MFE/MAE. Same-candle TP+SL остаётся `AMBIGUOUS/censored`. Формулы LONG/SHORT gross/net P&L, funding signs и grid completed-pair fees проверены зеркальными и path tests; подтверждённой знаковой инверсии не обнаружено.

Обновление не меняет схему БД и lineage моделей. Перед запуском достаточно остановить старую версию, сделать резервную копию БД и запустить v1.4.6; удалять market history, recommendations или outcomes не требуется.
...
Backend-подсказки также разделены по strategy family. Grid-рекомендация может получать советы о диапазоне, шагах сетки и сильном тренде; trend-рекомендация — только действия по подтверждению LONG/SHORT, восстановлению entry/TP/SL, single-position contract и first-touch evidence. Машинные блокировки одной стратегии не используются как объяснение другой. Legacy-рекомендации без `bot_type` трактуются как `futures_grid`, поскольку до появления trend все строки проекта были сеточными.

Frontend локализует основные grid/trend guard-коды, не прогоняет уже русские backend actions через повторную машинную «русификацию» и дедуплицирует повтор одного конкретного invariant-кода, пришедший из persisted и live validation. Изменение не затрагивает торговые формулы, outcome labels, calibrator lineage, router thresholds или схему БД.
...
Версия 1.2.0 добавляет `strategy-profitability-router-v1`. Для одного рыночного снимка система по-прежнему независимо строит кандидаты `futures_grid` и `directional_trend`, но больше не сравнивает их несопоставимые raw `score`. Router допускает кандидата к сравнению только при активном bot-specific LogReg/Platt calibrator, корректном exact-policy fingerprint, положительной selected-policy и terminal-holdout денежной ожидаемости, положительных row-level/temporal lower bounds, доступном expected shortfall и одинаковом 12-часовом return basis. Побеждает стратегия с большей risk-adjusted monetary utility; при недостаточном преимуществе обе остаются `no_trade`.

`futures_grid` остаётся сеточной стратегией. Его `long`/`short` варианты являются directional grid, а не обычной трендовой позицией. `directional_trend` создаёт одну long/short позицию с отдельными entry, TP и SL, без grid levels, усреднения и pyramiding. Победившая trend-рекомендация может пройти внутренний operator lifecycle и создать `bot_instance` типа `external_single_order_audit`, но сервис **не отправляет биржевой ордер**. Он формирует `directional-single-order-package-v1` для ручного либо внешнего execution layer; `recommendation_only=true` и `exchange_order_submitted=false` являются обязательными fail-closed полями.

Проигравший кандидат сохраняется как `shadow_competitor` и получает собственный strategy-specific outcome, поэтому обе модели продолжают обучаться на парных будущих периодах. Одновременно запустить grid и single-position trend по одному символу в one-way account нельзя: execution-preflight блокирует конфликт `SYMBOL_STRATEGY_ALREADY_RUNNING`. Реальная прибыльность, качество внешнего исполнения и live edge этой архитектурой не доказываются.

## История: отдельная directional trend-ветка в shadow-режиме (v1.1.0)
...
`directional_trend` создаёт один proxy-position с отдельными TP/SL, без grid levels, усреднения и pyramiding. В исторической поставке v1.1.0 он был только shadow/research. Начиная с v1.2.0 он может быть выбран meta-router и материализован как внутренний audit-instance с внешним single-order package; сам сервис биржевой ордер не отправляет.
...
Сервис собирает рыночные данные Bybit Linear USDT Futures / USDT Perpetual и рассчитывает multi-timeframe признаки для двух семейств: сеточного `futures_grid` и single-position `directional_trend`. Оба кандидата могут быть оценены strategy-profitability router; grid создаёт audit-пакет для внешнего grid execution, а trend — audit-пакет для одной внешней/ручной позиции. Сервис не отправляет ордера Bybit. Дополнительно может подключаться локальный LLM-reviewer по свечам; полный журнал решений и состояний хранится в выбранном backend: SQLite или PostgreSQL.
...
- для operator lifecycle поддерживаются `futures_grid` (`account_mode=unified`, `margin_mode=cross`, `grid_type=arithmetic`) и выбранный router-ом `directional_trend` (`account_mode=unified`, `margin_mode=isolated`, одна позиция); оба остаются recommendation/audit contracts без private Bybit order submission;
