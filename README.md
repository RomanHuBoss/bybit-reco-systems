...
- панель деталей переписана под ручной запуск: JSON убран из основной операторской зоны, вместо него выводятся копируемые поля для Bybit (`range`, `grid_levels`, `leverage`, `kill switch`, `TP/SL`); ссылки на график и страницу создания бота вынесены только в заголовок панели деталей и в таблицу;
...
- в верхней плашке деталей добавлен индикатор связи с BTC (`BTC`: `незав.`, `связь`, `BTC-led` с `r=...`), чтобы оператор сразу видел, насколько сигнал самостоятельный;
