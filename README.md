## Исследовательские outcomes без ослабления торговых gates (v1.0.70)

Версия 1.0.70 исправляет узкий дефект взаимодействия локального advisory-LLM и теневого контура исходов. Явно разрешённые, риск-чистые `shadow_no_trade` roots теперь созревают независимо от LLM verdict даже при `policy_evaluation_eligible=false`. Такие строки остаются `shadow_exploration`: они дают проверяемые proxy-outcomes, но не входят в exact-policy calibration, не увеличивают `calibrator_n` и не могут сделать рекомендацию исполнимой. Порог `MEAN_REVERSION_MIN_SCORE`, risk/economics gates, status semantics и recommendation/audit-only boundary не ослаблены.
