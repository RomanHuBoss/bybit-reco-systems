...
- **2026-04-04 — дополнительный аудит sentiment-контура, legacy-row sanitization и API-контракта.** Закрыт скрытый класс дефектов, при котором legacy/imported строки в таблице `sentiment` с `+/-Infinity` или некорректным `volume` могли искажать EWMA-агрегаты, искусственно рисовать экстремальный `risk_on/risk_off` и потенциально ломать operator-facing выдачу невалидными числами.
- Sentiment read/write слой переведён на fail-closed/skip-invalid поведение: запись `sentiment`/`velocity` теперь принимает только finite-значения, `volume` обязан быть `>= 0`, чтение и агрегаторы (`compute_sentiment_agg`, `compute_symbol_sentiment_map`) пропускают poisoned legacy rows и дополнительно clamp-ят finite outliers к ожидаемому диапазону `[-1, 1]`.
- Добавлены регрессионные тесты на: игнорирование legacy `Infinity` в global/symbol sentiment aggregation, запрет non-finite/negative payload'ов на DB/API-слое и защиту `/api/v1/sentiment` от отдачи отравленных исторических строк.
...
- Исправлен скрытый дефект re-entry внутри publication-chain: `active`-элемент той же цепочки больше не привязывается к уже **stopped** историческому боту как к будто бы валидному idempotent-исполнению. Если предыдущий chain-bot уже закрыт, новый `execute` создаёт новый running-бот; переиспользование старого bot_id разрешено только для ещё живой `running`-экспозиции.
...
- **2026-04-04 — дополнительный аудит семантической идемпотентности JSON payload.** Duplicate-detection для `trades` и `bot_instances` теперь основан на **канонизированном JSON**, а не на сырой строке `json.dumps(...)`. Повтор одного и того же trade/bot payload с другим порядком ключей в `meta` / `mode` / `params` / `state` больше не ломает idempotent retry и не даёт ложный `409 already exists with different payload`.
- `/api/v1/recommendations?snapshot=latest_operator` теперь уважает фактически запрошенный набор статусов. Если оператор отключил actionable-идеи и смотрит только `pending/blocked/no_trade/suppressed`, API больше не подтягивает более старый actionable snapshot лишь потому, что он был «операторски видимым»; выбирается свежайший snapshot, в котором реально есть запрошенные записи.
...
- технический аудит этой ревизии в `docs/audit_2026-04-04.md`.
