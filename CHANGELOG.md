...
- Pending rows now get a separate `Ждать LLM-review` title and explanatory text; `NO_TRADE` warning is emitted only for persisted `status=no_trade`.
- Bumped static asset cache key to `manual-ui-v22`.
- Added regression coverage in `tests/test_iteration140_ui_pending_not_no_trade.py`; validation: `455 passed`, `python -m compileall -q app tests main.py`, `node --check app/ui/static/app.js`.

## 2026-05-10 — Runtime grid step/count snapping fix

- Fixed false `GRID_STEP_LEVELS_MISMATCH` warnings/blockers after a fresh start on empty DB: strict generated Bybit arithmetic grids are now snapped as one `lower + grid_count * step` geometry instead of snapping range and step independently.
- The UI/execution payload now keeps `trade_plan.levels.range`, `params.price_range_*`, `operator_sheet.range_*`, `actual_grid_step_abs` and `actual_grid_spacing_pct` synchronized after Bybit `tick_size` alignment.
- `kill_switch.upper` is moved outward when range snapping expands the upper boundary, preserving the guard outside the tradable range.
- Added regression coverage in `tests/test_iteration127_tick_safe_grid_snapping.py`; validation: `448 passed`, `python -m compileall -q app tests`, `node --check app/ui/static/app.js`.
