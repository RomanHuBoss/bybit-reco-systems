...
Сервис теперь fail-closed отвергает `instruments-info` без точного совпадения `symbol`, что снижает риск валидации чужими лимитами, но не отменяет фундаментальное ограничение публичного REST как источника execution truth.
