...
| high | Политика плеча не соответствовала операторскому требованию: recommender выбирал 1–3x, что делало большинство идей экономически непригодными для указанного стиля торговли. | `app/risk.py`, `app/settings.py`, `.env.example`, `app/recommender.py` | Рекомендации формально валидны, но практически не соответствуют целевому профилю доходности; оператор мог запускать низкоплечевые сетки вопреки собственному правилу. | Добавлен `min_leverage` в risk-limits. Defaults: `min_leverage=5`, `max_leverage=5`. Recommender выбирает 5x только для сильных directional-условий; остальные идеи fail-closed блокируются `MIN_LEVERAGE_PER_BOT`. Явный `max_leverage` ниже 5 сохраняется как операторский cap и снижает effective `min_leverage` до cap, чтобы не нарушать верхний риск-лимит. | `tests/test_iteration152_deep_trading_reaudit.py::test_risk_limits_default_to_operator_minimum_five_x_and_fail_closed_bounds`, обновлены existing risk-limit tests |
...
- `tests/test_iteration152_deep_trading_reaudit.py`
  ...
  - `test_risk_limits_default_to_operator_minimum_five_x_and_fail_closed_bounds`
