...
`futures_grid` remains arithmetic grid mechanics, including neutral/long/short directional bias. `directional_trend` is not a grid: it is a single long/short position with one entry, TP and SL, no averaging, no pyramiding and no replacement-grid topology. The losing candidate is persisted as `shadow_competitor`, so strategy-specific outcomes continue to mature for both policies on paired future periods.

A selected trend plan may enter the internal operator/audit lifecycle. The service validates current Bybit public metadata, conservative tick/qty/leverage snapping, live-price drift and TP/SL exhaustion, spread, funding, notional/margin caps, daily loss budget and one-way symbol conflicts. It then creates an `external_single_order_audit` instance and a `directional-single-order-package-v1`. It does not call private Bybit order endpoints: `recommendation_only=true` and `exchange_order_submitted=false` must remain true. A manual or external execution layer is responsible for actual order submission and reconciliation.

The legacy identifiers `directional_trend_shadow_v1` and `directional_trend_label_v1` remain unchanged to preserve the v1.1.0 outcome/calibration lineage. The word `shadow` in the contract identifier is historical and is not an execution permission flag.

## Historical v1.1.0 contract: directional trend shadow policy
