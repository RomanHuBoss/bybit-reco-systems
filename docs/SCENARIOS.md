## Сценарий длительного low-edge рынка — v1.0.70

Если все инструменты получают `MEAN_REVERSION_EDGE_UNCONFIRMED`, оператор по-прежнему видит `НЕ ТОРГОВАТЬ`. Система сохраняет независимые риск-чистые shadow roots и после horizon строит исследовательские proxy-outcomes даже при включённом LLM reviewer. Эти исходы не переводят рекомендации в actionable и не входят в exact-policy calibration, пока кандидат не прошёл предкалибровочный candidate screen. Поэтому допустимо одновременно видеть рост общей истории outcomes и `calibrator_n=0`.
