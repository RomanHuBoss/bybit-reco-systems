...
Непрерывный минутный path подтверждает касание TP до любого касания SL. Outcome получает `TP_FIRST`; денежный результат считается по TP с costs/funding.

### SL достигнут первым

SL подтверждён раньше TP. Outcome получает `SL_FIRST`; положительное последующее движение после закрытия не переписывает событие.

### Ни одна граница не достигнута

На точной 12-часовой границе выполняется `HORIZON_EXIT`; сохраняются terminal price, net return, MFE/MAE. Прибыльный timeout не переименовывается в TP-first.

### Обе границы внутри одной 1m свечи

Порядок внутри OHLC неизвестен. Outcome получает диагностическое `AMBIGUOUS` и цензурируется: он не становится ни успехом, ни поражением и не входит в softmax fit.

### Выбор стратегии

Router допускает trend только при готовой exact-policy first-touch модели, валидных трёх вероятностях, консервативном преимуществе TP-first над SL-first и положительной денежной EV/lower bound. Иначе trend остаётся `no_trade`, а grid оценивается по собственному контракту. При недоказанности обоих вариантов итог — `НЕ ТОРГОВАТЬ`.
...
2. Цена пробивает верхнюю или нижнюю защитную границу.
3. Остаточная позиция закрывается по консервативному observed-extreme bound.
4. Итоговый `net_proxy_return` может остаться положительным.
5. `success` всё равно равен `0`, поскольку terminal protection была нарушена.
6. Журнал показывает «Неуспех · kill-switch», расчётный P&L и конкретную сторону/цену пробоя.

Legacy-строки без terminal diagnostics не реконструируются предположением.
